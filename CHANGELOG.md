...
## [2.0.0](https://github.com/joehe71/browser-tools/compare/v1.0.1...v2.0.0) (2026-09-21)
...
* warm npx cache and pin chrome-devtools-mcp version\n\nThe MCP Client connect timeout is ~20s. On a cold cache, npx's\npackage fetch alone blows past it, so the very first connect attempt\ntimes out even though the server itself is fine.\n\n- Pin chrome-devtools-mcp@1.9.0 instead of [@latest](https://github.com/latest) so npx skips the\n  registry round-trip on every spawn and we don't pick up breaking\n  upstream changes by accident.\n- Best-effort warm-up: run 'npx ... --version' before the first\n  register of a session to pre-populate the npx cache. Failures are\n  logged but never block registration.\n\nCloses the 'MCP server chrome-devtools connect timed out after 21000ms'\nerror reported on first launch. ([a82a0ec](https://github.com/joehe71/browser-tools/commit/a82a0ec4545ad881e7854c3538911a5191b0f3f1))
